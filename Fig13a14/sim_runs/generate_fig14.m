...
sinr_mat_evals = zeros([num_evals,numel(ant_sweeps),numel(algo_mask),4]);
clear eval_params_all
eval_idx=0;
seed_eval_idx=1;
while(eval_idx<num_evals)
    disp("Starting eval: "+num2str(eval_idx+1))
    [eval_params_all{eval_idx+1},sinr_mat_evals(eval_idx+1,:,:,:),ret_val] = ant_sweep_run(seed_eval_idx,algo_mask);
    if (ret_val==-1)
