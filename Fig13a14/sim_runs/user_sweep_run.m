function [eval_params, sinr_mat, ret_val] = user_sweep_run(seed,algo_mask)
...
sinr_mat = zeros([numel(user_sweep_vec),num_algos_sim,4]);

max_no_users = 8;
meta_params.num_users = max_no_users;
...
max_sim_settings_struct = load_sim_setting(meta_params, 1); % load geometrical setting
meta_params.max_users_cords = max_sim_settings_struct.sim_setting.selected_user_cords;

for num_users = user_sweep_vec
meta_params.num_users = num_users;
meta_params.oversamp_fac = meta_params.num_users;
% meta_params.oversamp_fac = 4;
meta_params.sampling_freq = meta_params.bandwidth*meta_params.oversamp_fac;
...
for i=1:1:num_algos_sim
    if(algo_mask(i)==1)
        sinr_mat(usr_cnt,i,1) = mean(eval_params{i}.sinr_calc,[1,2]);
        sinr_mat(usr_cnt,i,2) = min(mean(eval_params{i}.sinr_calc,[1]));
        sinr_mat(usr_cnt,i,3) = max(mean(eval_params{i}.sinr_calc,[1]));
        sinr_mat(usr_cnt,i,4) = std(mean(eval_params{i}.sinr_calc,[1]));
    end
end
