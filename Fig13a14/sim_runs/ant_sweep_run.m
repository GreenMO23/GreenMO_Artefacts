function [eval_params, sinr_mat, ret_val] = ant_sweep_run(seed,algo_mask)
...
ant_sweep_vec = 8:8:64;

rand_placement = 1;
meta_params.plot_setting = false;
meta_params.bandwidth = 20e6;

meta_params.user1_SNR = 5; % db
meta_params.tx_pow = 20; % dbm

meta_params.plot_setting=true;

% user_sweep_vec = 2:2:8;

ant_cnt=1;

sinr_mat = zeros([numel(ant_sweep_vec),num_algos_sim,4]);


meta_params.num_users = 8;
meta_params.oversamp_fac = meta_params.num_users;

% meta_params.oversamp_fac = 4;

meta_params.num_ants = max(ant_sweep_vec);
meta_params.sampling_freq = meta_params.bandwidth*meta_params.oversamp_fac;
% init_sim_settings_struct = load_sim_setting(meta_params, 1); % load geometrical setting

% meta_params.init_users_cords = init_sim_settings_struct.sim_setting.selected_user_cords;
...
[maxant_rx_interfered_waveforms, maxant_rx_interfered_waveforms_no_noise,...
    maxant_rx_interfered_waveforms_dig, maxant_rx_interfered_waveforms_babf]...
    = apply_td_chan(meta_params,tx_waveforms, loopback, channel_struct); % Convolve the channel and apply path loss to get the RX waveforms

disp("Channel applied")

for num_ants = ant_sweep_vec

meta_params.num_ants = num_ants;
channel_struct.num_ants = num_ants;

clear rx_interfered_waveforms rx_interfered_waveforms_no_noise...
    rx_interfered_waveforms_dig rx_interfered_waveforms_babf;

rx_interfered_waveforms ={};
rx_interfered_waveforms_no_noise ={};
rx_interfered_waveforms_dig ={};
rx_interfered_waveforms_babf ={};

for i=1:1:num_ants
rx_interfered_waveforms{i} = maxant_rx_interfered_waveforms{i};
rx_interfered_waveforms_no_noise{i} = maxant_rx_interfered_waveforms_no_noise{i};
rx_interfered_waveforms_dig{i} = maxant_rx_interfered_waveforms_dig{i};
rx_interfered_waveforms_babf{i} = maxant_rx_interfered_waveforms_babf{i};
end
% disp("Channel applied")
...
if(algo_mask(1)==1)
    ...
    if(ret_val1==-1)
        sinr_mat = zeros([num_algos_sim,4]);
        eval_params=0;
        ret_val=-1;
        return
    end
...
for i=1:1:num_algos_sim
    if(algo_mask(i)==1)
        sinr_mat(ant_cnt,i,1) = mean(eval_params{i}.sinr_calc,[1,2]);
        sinr_mat(ant_cnt,i,2) = min(mean(eval_params{i}.sinr_calc,[1]));
        sinr_mat(ant_cnt,i,3) = max(mean(eval_params{i}.sinr_calc,[1]));
        sinr_mat(ant_cnt,i,4) = std(mean(eval_params{i}.sinr_calc,[1]));
    end
end
