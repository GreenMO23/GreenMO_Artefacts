...
if(unsynch)
    ...
    op_struct.user_lts_offsets = op_struct.lts_ind(1,1:params.num_users)-op_struct.lts_ind(1,1);
end
length_rx_samples = max(length(raw_rx_dec)-first_lts_ind-params.TX_NUM_SAMPS+1,1);%-2*1.5*params.TX_NUM_SAMPS; %max index for the start of a packet
...
%         lts_indices_rel_first_user = lts_ind_loop-lts_ind_loop(1)+1;
        if(unsynch)
            ...
            op_struct.user_lts_offsets = params.lts_offsets;
        end
    catch
