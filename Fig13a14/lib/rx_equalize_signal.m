function syms_eq_mat = rx_equalize_signal(rx_vec_cfo_corr,rx_H_est,param,lts_offsets,user_idx)
...
payload_ind = 1+lts_offsets(user_idx+1)+((param.NUM_LTS+0.5)*param.N_SC+param.inter_user_zeros)*(param.num_users-user_idx);
% plot(abs(rx_vec_cfo_corr))
% hold on
% xline(payload_ind)
...
rx_H_est=rx_H_est(:);
syms_eq_mat = syms_f_mat ./ (repmat(rx_H_est, 1, param.N_OFDM_SYMS)+1e-20);
