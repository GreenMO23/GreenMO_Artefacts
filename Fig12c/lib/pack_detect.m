...
%     lts_range_possibilities = ((params.NUM_LTS+0.5)*params.N_SC+params.inter_user_zeros)*(params.num_users)+params.inter_user_zeros;
    lts_range_possibilities_oneuser_min = ((params.NUM_LTS+0.5)*params.N_SC+params.inter_user_zeros);
    lts_range_possibilities_oneuser_max = ((params.NUM_LTS+0.5)*params.N_SC+params.inter_user_zeros)+params.inter_user_zeros*0.75;
    ...
    while(peak_idx<=numel(sing_user_lts_peaks_abs_idx))
        ...
        possiblemultipath_peaks = ((sing_user_lts_peaks_abs_idx-curr_peak)>0)...
                & ((sing_user_lts_peaks_abs_idx-curr_peak)<params.N_SC); %Remove peaks withi one frame
