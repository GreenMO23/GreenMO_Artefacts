function [avg_sinr_across_users]...
           = analyse_data_greenmo(posn_index,data_dir, test_ids,lts_thresh)
if(nargin==3)
    lts_thresh=0.6;
end
...
%     disp("Expmt: GreenMO"+", Test idx: "+num2str(test_idx)+", Users: "+num2str(num_users));
...
%                 tic
        if(ind==1)
            [op_struct_cell{ind}, ofdm_params_out{ind}] = ofdm_rx(rx_samples_resamped{ind}, ofdm_params,[],lts_thresh);
...
%     end
    ...
    lts_len_singuser = (((ofdm_params.NUM_LTS+0.5)*sing_lts_len));
    ...
    pkt_and_payload_plus_zeros = (ceil((lts_len+payload_len)/ofdm_params.N_SC)+1)*ofdm_params.N_SC+ofdm_params.inter_user_zeros*0.5;
    ...
    for pkt_idx=1:1:num_packets
       for i=1:1:samp_fac
           lts_offsets = op_struct(i).user_lts_offsets;
...
%            end
           ...
           rel_lts_ind = op_struct(i).packet_lts_list(pkt_idx,:)-pkt_lts_ind+1;
           lts_offset=0;
           half_lts = num_subc/2;
           if(plot_debug)
               figure(10)
    ...
    %                 (num_packets,ofdm_params.NUM_LTS,num_users,samp_fac,num_subc)
                   rx_lts(pkt_idx,lts_idx,user_idx,i,:) = all_lts(lts_start_ind_offseted+half_lts+(lts_idx-1)*num_subc:lts_start_ind_offseted+half_lts+(lts_idx)*num_subc-1);
               end
...
%         evm_calc = zeros(num_packets,num_users);
    ...
    for pkt_idx=1:1:num_packets
        ...
        for user_idx=1:1:num_users
            ...
            [decode_op_structs(user_idx),decode_params(user_idx)]= ofdm_decode(squeeze(curr_payload).', ofdm_params_tx(user_idx), ofdm_params,user_idx-1, lts_offsets);
...
%       % what we need for each expmt
    ...
    avg_sinr_across_users = [avg_sinr_across_users cleaned_sinrs_per_packet.'];
...
function cleaned_vec = remove_outliers(data_vec)
    ...
    % For data consistency with other method of calculating SINR
