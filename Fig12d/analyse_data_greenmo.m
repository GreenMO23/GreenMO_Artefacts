function [avg_evm_snr_users_across_packets,min_evm_snr_users_across_packets,...
           ...
           = analyse_data_osamp(expmt_mode, data_dir, test_ids,lts_thresh)
if(nargin==3)
    lts_thresh=0.6;
end
...
tx_params_dir = data_dir+"/"+expmt_mode+"/tx_data"+num2str(num_users)+"/";
load(tx_params_dir+"ofdm_params_tx.mat");




path_dir = data_dir+"/"+expmt_mode+"/";
...
for test_index = 1:1:num_tests
    ...
    fname = path_dir+"rx_samps_bit_phase_"+num2str(test_idx);
    load(fname);
...
save(path_dir+"/decode_out_"+expmt_mode+".mat",'ber_dict','evm_snr_dict','sinr_dict','tdma_snrs');
