...
% Correlate for LTS
if(~lts_predetec)
    [first_lts_ind] = pack_detect(raw_rx_dec,params,lts_thresh, unsynch);
...
op_struct.packet_payload_list = zeros(params.num_packets);
op_struct.packet_lts_list = zeros(params.num_packets);
...
for pidx = 1:length(pack_ind_list)
    ...
    try
        ...
        rx_pkt_and_preamble_range = lts_ind_loop:(lts_ind_loop+((params.NUM_LTS+0.5)*(params.N_SC)+params.inter_user_zeros)*params.num_users+tx_num_origsamp-1);
...
%         xline(max(rx_pkt_and_preamble_range)-(min(rx_pkt_and_preamble_range)-1000))
        [rx_H_est,rx_vec_cfo_corr,params]= extractChannel(raw_rx_dec_iter,1,params); 
    catch
