function [op_struct, params] = ofdm_rx_siso(rx_vec_air, params, tx_params,lts_ind)
...
% Correlate for LTS
if(~lts_predetec)
    [lts_ind] = pack_detect(raw_rx_dec,params);
    op_struct.lts_ind = lts_ind;
else
    op_struct.lts_ind = lts_ind;
end
...
%% NEW THINGS
length_rx_samples = length(raw_rx_dec)-lts_ind-params.TX_NUM_SAMPS+1;%-2*1.5*params.TX_NUM_SAMPS; %max index for the start of a packet
lts_ind_list = lts_ind + (0:params.TX_NUM_SAMPS: length_rx_samples);
...
op_struct.rx_H_est_vec = zeros(params.N_SC,params.num_packets);
op_struct.rx_syms = zeros(params.N_OFDM_SYMS*params.N_DATA_IND,params.num_packets);
op_struct.rx_data = zeros(params.N_OFDM_SYMS*params.N_DATA_IND,params.num_packets);
op_struct.evm_mat = zeros([size(params.tx_syms_mat), params.num_packets]);
op_struct.aevms = zeros(1,params.num_packets);
op_struct.snr = zeros(1,params.num_packets);
% op_struct.bernum = zeros(1,params.num_packets);
% op_struct.berratio = zeros(1,params.num_packets);
if(lts_ind<0)
%     error('LTS correlation not found')
    op_struct =[];
    params=[];
    return
end
for pidx = 1:length(pack_ind_list)
    ...
    try
        ...
        rx_pkt_and_preamble_range = lts_ind_loop:(lts_ind_loop+params.inter_user_zeros+(params.NUM_LTS+0.5)*params.N_SC+tx_num_origsamp-1);
        raw_rx_dec_iter = raw_rx_dec(rx_pkt_and_preamble_range);
        [rx_H_est,rx_vec_cfo_corr,params]= extractChannel_siso(raw_rx_dec_iter,1,params); 
    catch
        op_struct =[];
        params=[];
        return
    ...
    if(params.DO_DECODE)
        %% Rx Payload
        syms_eq_mat = rx_equalize_signal_siso(rx_vec_cfo_corr,1,op_struct.rx_H_est_vec(:,pidx),params);
        ...
        op_struct.snr(pidx) = 10*log10(1./op_struct.aevms(pidx));
